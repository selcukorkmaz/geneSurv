shinyServer(function(input, output, session) {
  ...
  nearZeroFeatures <- reactive({
    if (input$nearZero && input$runProcess){
      ...
      if(!is.null(dataN()$features)){
        nearZeroFeatures = as.data.frame(dataN()$data)
  ...
  output$nearZeroFeaturesResult <- DT::renderDataTable(server = FALSE, {
  ...
  observe({
    if(input$preProcess){
      updateSelectizeInput(session, "excludeVarsProcess", choices = colnames(dataM()), selected = input$excludeVarsProcess)
  ...
  observe({
    updateSelectizeInput(session, "statusVariableKM", choices = colnames(dataM()), selected = colnames(dataM())[2])
  })
