shinyServer(function(input, output, session) {
  ...
  dataN <- reactive({
    ...
    if(input$preProcess && input$runProcess){
      ...
      if(!is.null(input$excludeVarsProcess)){
        
        data = dataL[!(colnames(dataL) %in% c(input$survivalTimeProcess, input$statusVariableProcess, input$excludeVarsProcess))]
        data2 = dataL[(colnames(dataL) %in% c(input$survivalTimeProcess, input$statusVariableProcess, input$excludeVarsProcess))]
        
        
      }else{
        
        data = dataL[!(colnames(dataL) %in% c(input$survivalTimeProcess, input$statusVariableProcess))]
        data2 = dataL[(colnames(dataL) %in% c(input$survivalTimeProcess, input$statusVariableProcess))]
        
        
      }
      
      if (input$nearZero && input$runProcess){
        nZ = nearZeroVar(data, saveMetrics = FALSE)
        if(length(nZ) != 0){
          nearZeroFeatures = nearZeroVar(data, saveMetrics = FALSE, names = TRUE)
          data = data[-nZ]
        }
      }
      ...
      if (input$scale && input$runProcess){
        
        data = scale(data, center = FALSE, scale = TRUE)
        
      }
      
      if (input$transform && input$runProcess){
        
        data = log(data+abs(min(data, na.rm=TRUE))+1)
        
      }
      
      dataNew = list(data = cbind.data.frame(data2, data), features = nearZeroFeatures)
      
    }else{
      
      dataNew = list(data = dataLoad(), features = NULL)
  ...
  nearZeroFeatures <- reactive({
    if (input$nearZero && input$runProcess){
      
      if(!is.null(dataN()$features)){
        nearZeroFeatures = as.data.frame(dataN()$features)
        colnames(nearZeroFeatures) = "Near zero features"
        return(nearZeroFeatures)
      }else{
  ...
  output$nearZeroFeatures <- DT::renderDataTable(server = FALSE, {
    
    if (input$nearZero && input$runProcess){
      datatable(nearZeroFeatures(), extensions = 'ColReorder', options = list(colReorder = TRUE))
      
    }
  })
  
  output$dataUpload <- DT::renderDataTable(server = FALSE, {
  ...
  observe({
    ...
    if(input$preProcess){
      updateSelectizeInput(session, "survivalTimeProcess", choices = colnames(dataM()), selected = colnames(dataM())[1])
    }
  })
  
  observe({
    if(input$preProcess){
      updateSelectizeInput(session, "statusVariableProcess", choices = colnames(dataM()), selected = colnames(dataM())[2])
  ...
  observe({
    if(input$preProcess){
      updateSelectizeInput(session, "excludeVarsProcess", choices = colnames(dataM()), selected = NULL)
      
    }
  })
  
  # observe({
  #   updateSelectizeInput(session, "excludeVarsProcess", choices = colnames(dataM()[!(colnames(dataM()) %in% c(input$survivalTimeProcess, input$statusVariableProcess))]), selected = NULL)
  # })
  ...
  observe({
    updateSelectizeInput(session, "statusVariableKM", choices = colnames(dataM()), selected = colnames(dataM())[1])
  })
  
  observe({
    updateSelectizeInput(session, "survivalTimeKM", choices = colnames(dataM()), selected = colnames(dataM())[2])
  })
  ...
  observe({
    ...
    if (!is.null(data_tmp_rCox)){
      updateSelectizeInput(session = session, inputId = "statusVariablerCox", choices = colnames(data_tmp_rCox), selected = colnames(data_tmp_rCox)[2])
    } else {
  ...
  observe({
    updateSelectizeInput(session, "survivalTimeCutoff", choices = colnames(dataM()), selected = colnames(dataM())[1])
  })
  ...
  output$descriptives <- DT::renderDataTable(server = FALSE, {
  ...
  output$lifetableResult <- DT::renderDataTable(server = FALSE, {
  ...
  output$medianLifeTimeResult <- DT::renderDataTable(server = FALSE, {
  ...
  output$hazardRatioResult <- DT::renderDataTable(server = FALSE, {
  ...
  output$comparisonTestResults <- DT::renderDataTable(server = FALSE, {
  ...
  resultKM <- reactive({
    ...
    if(input$runKM){
      ...
      isolate({
        ...
        withProgress(message = 'Kaplan-Meier in progress...',  detail = 'This may take a while...', value = 1,{
  ...
  output$descriptivesKM <- DT::renderDataTable(server = FALSE, {
  ...
  output$survivaltableResult <- DT::renderDataTable(server = FALSE, {
  ...
  output$survivalPlot <- highcharter::renderHighchart({
    ...
    if(input$factorVarKM){
      ...
      surv = resultKM()$testResult$survivalTable
    }
  ...
  output$meanMedianSurvivalTimesResult <- DT::renderDataTable(server = FALSE, {
  ...
  output$hazardRatioResultKM <- DT::renderDataTable(server = FALSE, {
  ...
  output$hazardPlot <- highcharter::renderHighchart({
    ...
    if(input$factorVarKM){
      ...
      hazard = resultKM()$testResult$hazardRatio
      
    }
    highchart() %>% hc_exporting(enabled = TRUE, filename = "hazardPlot") %>% 
      hc_add_series(name = "Hazard", type = "line", data = hazard$Hazard, showInLegend = FALSE, zIndex = 1, marker = list(lineColor = "black", lineWidth = 1), lineWidth = 0, id = "hazard") %>%
  ...
  output$comparisonTestResultsKM <- DT::renderDataTable(server = FALSE, {
  ...
  resultCox <- reactive({
    ...
    if(input$runCox){
      ...
      isolate({
        ...
        withProgress(message = 'Cox Regression in progress...',  detail = 'This may take a while...', value = 1,{
    ...
    if(input$selectPlotCox == 1 && input$runCox){
      ...
      for (i in 1:var) {
        ...
        if(input$changeThemeSchoenfeld == "theme0"){
          ...
          sp = highchart() %>% hc_exporting(enabled = TRUE, filename = "Schoenfeld plot") %>% 
  ...
  randomForestSurvival <- reactive({
    ...
    if(input$runRF){
      ...
      isolate({
        ...
        withProgress(message = 'Random Survival Forests in progress...',  detail = 'This may take a while...', value = 1,{
          ...
          if(input$selectAllVarsRF){
            
            indx = !(colnames(dataM()) %in% c(input$survivalTimeRF, input$statusVariableRF))
            continuousInput = colnames(data.matrix(dataM()[,indx, drop = FALSE]))
            categoricalInput = NULL
  ...
  output$survival <- DT::renderDataTable(server = FALSE, {
  ...
  output$survivalOOB <- DT::renderDataTable(server = FALSE, {
  ...
  output$errorRate <- DT::renderDataTable(server = FALSE, {
  ...
  output$chf <- DT::renderDataTable(server = FALSE, {
  ...
  output$chfOOB <- DT::renderDataTable(server = FALSE, {
  ...
  featureSelectRF <- reactive({
    ...
    if(input$varImp  && input$runRF){
      ...
      isolate({
        ...
        withProgress(message = 'Feature selection in progress...',  detail = 'This may take a while...', value = 1,{
          ...
          data = newData
          dropRate = input$dropOutRate
          
          # rsfInitial <- rfsrc(Surv(OS_Time, OS_Status) ~ ., data = data, ntree = 1000, tree.err=TRUE, importance = TRUE)
          rsf = randomForestSurvival()
          varImp = cbind.data.frame(var = names(rsf$importance), importance = rsf$importance)
          ...
          while((ncol(data)-2) > 2 && excludeVarNumber !=0){
            ...
            data = data[,c("survivalTime", "statusVar", rownames(varsInModel))]
            varImp = varImp[rownames(varsInModel),]
            
            # rsf <- rfsrc(Surv(OS_Time, OS_Status) ~ ., data = data, ntree = 1000, tree.err=TRUE, importance = FALSE)
            rsf = rfsrc(formula = Surv(survivalTime, statusVar) ~ .,  data = data, ntree = input$ntree, bootstrap = input$bootstrap, tree.err=TRUE, 
                        importance = FALSE, membership = TRUE, statistics = TRUE, do.trace = TRUE,
                        mtry = input$mtry, nodesize = input$nodesize, nodedepth = input$nodedepth, splitrule = input$splitrule, nsplit = input$nsplit,
                        split.null = FALSE, na.action = input$naAction, nimpute = input$nimpute, proximity = input$proximity, sampsize = NULL,
                        samptype = input$samptype, case.wt = NULL, xvar.wt = NULL, forest = TRUE, var.used = FALSE, 
                        split.depth = FALSE, seed = 1234, coerce.factor = NULL)
  ...
  output$variableImportance <- DT::renderDataTable(server = FALSE, {
    
    if(input$varImp  && input$runRF){
      
      bestFeatures = featureSelectRF()
      
      colnames(bestFeatures) = c("Feature", "Importance")
      
      datatable(bestFeatures, rownames = FALSE, extensions = c('Buttons','KeyTable', 'Responsive'), options = list(
        dom = 'Bfrtip', 
        buttons =            list('copy', 'print', list(             extend = 'collection',             buttons = c('csv', 'excel', 'pdf'),             text = 'Download'           )), keys = TRUE
      ))
  ...
  output$variableImportancePlot <- renderHighchart({
    ...
    if(input$varImp  && input$runRF){
      importance = featureSelectRF()[-1]
      
      
      highchart() %>% hc_exporting(enabled = TRUE, filename = "Selected features") %>% 
        hc_chart(type = "column", inverted = TRUE) %>% 
        hc_xAxis(categories = rownames(importance)) %>% 
        hc_add_series(data = importance[,1], name = "Importance") %>%
        hc_tooltip(crosshairs = TRUE, shared = TRUE, headerFormat = "<b>Time: </b>{point.x} <br>")%>%
        hc_add_theme(hc_theme_google())
    }
  })
  ...
  regularCoxResult <- reactive({
    ...
    if(input$runRegularized){
      isolate({
        ...
        withProgress(message = 'Penalized Cox Regression in progress...',  detail = 'This may take a while...', value = 1,{
          ...
          set.seed(1234)
          cvFit = cv.glmnet(x, y, family = "cox", alpha = input$rAlpha, nfolds = as.numeric(input$nFold))
  ...
  output$regularCox <- DT::renderDataTable(server = FALSE, {
    
    if(input$runRegularized){
      datatable(regularCoxResult()[[1]], extensions = 'Buttons', options = list(
        dom = 'Bfrtip',
        buttons = 
  ...
  output$regularizedPlot <- renderHighchart({
    ...
    if(input$runRegularized){
      ...
      isolate({
        ...
        withProgress(message = 'Cross Validation Curve in progress...',  detail = 'This may take a while...', value = 1,{
          ...
          lambda =  data.frame(sort(round(log(cvFit$lambda),4)))
          colnames(lambda) = "lambda"
          lambdaMin = round(log(cvFit$lambda.min),4)
          lambdaSe = round(log(cvFit$lambda.1se),4)
          
          lambdaMin2 = which(lambda$lambda == lambdaMin)
          lambdaSe2 = which(lambda$lambda == lambdaSe)
          
          highchart() %>% hc_exporting(enabled = TRUE, filename = "lambdaPlot") %>% 
            hc_add_series(name = "CI", type = "line", data = rev(cvFit$cvm), showInLegend = FALSE, zIndex = 1, marker = list(lineColor = "black", lineWidth = 1), lineWidth = 0, id = "survival") %>%
            hc_add_series(name = "CI", data = as.matrix(cbind(rev(cvFit$cvlo), rev(cvFit$cvup))),type = "errorbar", names = "Limits", showInLegend = FALSE, zIndex = 0, lineWidth = 1.5, linkedTo = "survival") %>%
            hc_chart(zoomType = "xy", inverted = FALSE) %>%
            hc_xAxis(categories = sort(round(log(cvFit$lambda), 1)), title = list(text = "log(Lambda)")) %>%
            hc_xAxis(plotBands = list(list(from = lambdaMin2, to = lambdaSe2, color = "rgba(100, 0, 0, 0.1)")))%>%
