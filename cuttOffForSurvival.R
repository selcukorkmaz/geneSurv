cuttOffForSurvival <- function(markers, survivalTime, statusVariable, status, compTest = "logRank", method = "significance", p= 1, q = 1, nmin=1,
  ...
  tryCatch(
    ...
    error=function(cond) {
      stop("Survival time variable is not appropriate, please check your Survival time.")
    }
  )
  ...
  error=function(cond) {
    stop("Please check your marker variable. There is not enough points to determine an optimal cutoff.")
  })
